...
import { WGCore, type PeerDTO } from "../components/wgcore.js";
...
export default class {
    ...
    private wgcore: WGCore;
    private admins: Admins
    private tokens: { [value: string]: 1 } = {}
    private state: State = {}
    ...
    private async enterScene(chatId: number, sceneString: string) {
        ...
        switch (sceneName) {
            ...
            case "get_admins":
                ...
                let keyboard1: InlineKeyboardButton[][] = []
            ...
            case "client_info":
                ...
                let banOrUnbanButton: InlineKeyboardButton;
    ...
    private checkLifetime(lifetime: number) {
        if (Date.now() > lifetime) return false
        return true
    }

    private getDateString(dateInt: number): string {
        const date = new Date(dateInt)
        return `${date.getDate() || "1"}.${date.getMonth() || "1"}.${date.getFullYear()}`
    }

    private getBoolToString(bool: boolean): string {
        if (bool) return "Да"
        return "Нет"
    }
