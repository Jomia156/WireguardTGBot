...
export class WGCore {
    ...
    getPeer(peerId: string) {
        ...
        if (!peersJSON[peerId]) throw new CustomError("Пользователь не найден.")
    ...
    createPeer(name: string, lifetime: number): PeerDTO {
        ...
        if (!newId) throw new CustomError("Достигнут лимит пользователей")
    ...
    updatePeer(peerId: string, data: UploadPeerDTO) {
        this.getPeer(peerId)
        let peersJSON = this._getPeersJSON()
    ...
    _genPeerConfig(peer: PeerDTO, banned = false) {
        if (!banned) {
            let peerString = "\n[Peer]"
